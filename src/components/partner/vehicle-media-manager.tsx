...
import {
  addVehicleMediaAction,
  removeVehicleMediaAction,
} from "@/domains/vehicles/actions";
...
export function VehicleMediaManager({
  ...
  function handleUpload(items: { id: string; url: string }[]) {
    ...
    if (newest) {
      startTransition(async () => {
        await addVehicleMediaAction(locale, organizationId, vehicleId, newest.url);
      });
    }
  }
  ...
  function handleRemove(mediaId: string) {
    startTransition(async () => {
      await removeVehicleMediaAction(locale, organizationId, vehicleId, mediaId);
    });
  }
